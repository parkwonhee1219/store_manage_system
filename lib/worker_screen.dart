...
class _WorkerManageState extends State<WorkerManage> {
  ...
  Widget build(BuildContext context) {
    ...
    // 급여 다이얼로그를 표시하는 함수
    void _showSalaryDialog(BuildContext context, Worker worker) {
      ...
      String formattedSalaryYes33 = NumberFormat('#,###,##0원').format(SalaryYes33);
      String formattedHourlyRate = NumberFormat('#,###,##0원').format(worker.hourlyRate);
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            ...
            content: Column(
              ...
              children: [
                Text(
                    ' ${worker.duty33 ? formattedSalaryYes33 : formattedSalaryNo33} ',style: TextStyle(fontSize: 20),),
                SizedBox(height: 20,),
                Text('시급: ${formattedHourlyRate}'),
                Text('총 근무 시간: ${worker.monthlyHours} 시간'),
                Text(
                    '3.3% 세금 적용 (${worker.duty33 ? "O" : "X"})'), // duty33 값 표시
              ],
            ),
            actions: [
              TextButton(
          ...
          return ListTile(
            ...
            subtitle: Column(
              ...
              children: List.generate(day_length, (dayIndex) {
                ...
                return Text(
                  '${workHour.day} ${workHour.startTime.hour}:${workHour.startTime.minute.toString().padLeft(2, '0')} - ${workHour.endTime.hour}:${workHour.endTime.minute.toString().padLeft(2, '0')}',
            ...
            trailing: Row(
              ...
              children: [
                IconButton(
                  ...
                  onPressed: () {
                    // 달라 버튼 클릭 시의 동작 정의
                    _showSalaryDialog(context, worker);
                  },
                ),
                IconButton(
